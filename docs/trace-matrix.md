...
|---|---|---|---|---|---|---|---|
| A1 | Discover mistakes from transcripts | Data ingestion, Failure mining | transcript import contract, normalized transcript importer, session-log adapter, plugin runtime mining path, onboarding backfill path | Phase 6 | In progress | Skylar | Current slices are the file-based transcript import MVP in `docs/plan-a1-transcript-import-contract-mvp.md`, the real-session adapter MVP in `docs/plan-p1-real-session-log-adapter-mvp.md`, the plugin runtime mining path MVP in `docs/plan-p1-runtime-mining-path-mvp.md`, and the onboarding backfill MVP in `docs/plan-p1-backfill-mining-onboarding-mvp.md`, extending ingestion toward actual OpenClaw session logs inside the plugin path. |
...
| C1 | Generate minimal repro eval | Eval generation | approved-candidate to eval-case converter, eval-case artifact CLI, plugin promotion policy | Phase 6 | In progress | Skylar | Current slices are the deterministic C1 MVP described in `docs/plan-c1-minimal-repro-eval-mvp.md` and the plugin promotion MVP in `docs/plan-p1-candidate-promotion-mvp.md`, converting approved candidates into inspectable eval-case artifacts through the plugin surface. The runtime-failure path is already proven separately through `docs/plan-e2e-runtime-failure-review-loop-mvp.md`. |
...
| D3 | Catch regressions before rollout | Eval runner, Results analytics | regression report, deterministic gate decision, local CLI, plugin eval tools | Phase 6 | In progress | Skylar | Current slices are `docs/plan-d3-lite-regression-gate-mvp.md` and `docs/plan-p1-eval-tools-mvp.md`, extending local regression/comparison capabilities into the plugin surface. |
